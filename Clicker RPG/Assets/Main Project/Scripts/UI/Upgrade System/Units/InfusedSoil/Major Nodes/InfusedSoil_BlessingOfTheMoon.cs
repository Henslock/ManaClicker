...
public class InfusedSoil_BlessingOfTheMoon : MajorUpgradeNode
{
    ...
    public InfusedSoil_BlessingOfTheSun majorSunBlessing;

    InfusedSoil_BlessingOfTheMoon()
    {
    ...
    public override void Purchase() 
    {
        ...
        if (isActive && !isPurchased)
        {
            if (GameManager.Instance.manaAmount >= unitUpgradeController.CalculateCost() && unitUpgradeController.GetUpgradeUnitRank() >= requiredRank)
            {
                ...
                //Deactive opposite node
                majorSunBlessing.Deactivate();
            }
        }
    }
    ...
    {
        if (majorSunBlessing.isPurchased)
        {
            return;
        }
        else
    ...
    {
        if (isPurchased)
            return "<size=18>BLESSING OF THE MOON:\n<color=#00FF50>Your Infused Soil is <color=#FFFFFF>50%</color> more efficient during the <color=#FFFFFF>Night</color>. You may only have one blessing at a time.</color>";
    ...
    {
        return "<color=#00FF50>+ Blessing of the Moon - your Infused Soil is 50% more efficient during the Night.</color>\n";
    }
    ...
    public override void ApplyMajorBonus() 
    {
        if(upgradeController.ManaUnit is InfusedSoilUnit)
        {
            ...
            mUnit.chosenBlessing = InfusedSoilUnit.ChosenBlessingEnum.MOON;
        }
    }
