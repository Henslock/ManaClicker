...
public class RuinsMajor_PrimordialWager : MajorUpgradeNode
{
    ...
    {
        if (isPurchased)
            return "<size=18>PRIMORDIAL WAGER <NYI>:\n<color=#00FF50>The cost to upgrade the Research Limit for every unit is reduced by <color=#FFFFFF>50%</color>.</color>";


        return "<size=18>PRIMORDIAL WAGER <NYI>:\n<color=#5E5E5E>The cost to upgrade the Research Limit for every unit is reduced by 50%.</color>";
    }

    public override string FetchUpgradeListText()
    {
        return "<color=#00FF50>+ Research Limit upgrade cost reduced by 50%.</color>\n";
    }
