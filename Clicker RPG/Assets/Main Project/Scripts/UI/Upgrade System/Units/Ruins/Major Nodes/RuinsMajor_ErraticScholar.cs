...
public class RuinsMajor_ErraticScholar : MajorUpgradeNode
{
    ...
    {
        if (isPurchased)
            return "<size=18>ERRATIC SCHOLAR <NYI>:\n<color=#00FF50>The cooldown to swap researched units is reduced by <color=#FFFFFF>80%</color>.</color>";


        return "<size=18>ERRATIC SCHOLAR <NYI>:\n<color=#5E5E5E>The cooldown to swap researched units is reduced by 80%.</color>";
    }

    public override string FetchUpgradeListText()
    {
        return "<color=#00FF50>+ The cooldown to swap researched units is reduced by 80%.</color>\n";
    }
