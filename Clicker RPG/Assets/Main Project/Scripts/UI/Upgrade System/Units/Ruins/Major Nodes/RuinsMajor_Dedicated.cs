...
public class RuinsMajor_Dedicated : MajorUpgradeNode
{
    ...
    {
        if (isPurchased)
            return "<size=18>DEDICATED <NYI>:\n<color=#00FF50>Your current researched unit operates at <color=#FFFFFF>25%</color> more efficiency.</color>";


        return "<size=18>DEDICATED <NYI>:\n<color=#5E5E5E>Your current researched unit operates at 25% more efficiency.</color>";
    }

    public override string FetchUpgradeListText()
    {
        return "<color=#00FF50>+ Your current researched unit operates at 25% more efficiency.</color>\n";
    }
